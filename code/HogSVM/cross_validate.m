...
function mean_acc = cross_validate(tr_images, tr_labels, nfold, tr_identity)
...
%         testi_ids = foldids{i};
%         
%         predi = HogSVM([w w],tr_images(:, :, traini_ids), tr_labels(traini_ids), tr_images(:, :, testi_ids));
%         
...
for i=1:nfold
  ...
  predi = HogSVM([2 2],tr_images(:, :, traini_ids), tr_labels(traini_ids), tr_images(:, :, testi_ids));
